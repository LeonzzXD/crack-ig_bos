...
class instagram:
 ...
 {SE}â”‚ [white]RESULT/[green]BERHASIL-{day}.txt[/]{SE}  â”‚ {SE}â”‚ [white]RESULT/[yellow]{K2}GAGAL-{day}.txt[/] {SE}â”‚
			...
			with ThreadPoolExecutor(max_workers=30) as shinkai:
				for i in user:
					try:
						...
						pass
			print('\n')
			prints(Panel(f" {P2}Hasil {acakrich}{len(internal)} {P2}username selesai hasil Ok : {H2}{len(BERHASIL)}{P2} Hasil Cp : {K2}{len(GAGAL)}{P2} ",width=80,padding=(0,8),style=f"{color_table}"))
		exit()
...
def lisensi():
 ...
 if status ==cek:
  li()
  cetak(nel('\t[green] SELAMAT LISENSI ANDA BERHASIL[/green]'))
  time.sleep(2)
